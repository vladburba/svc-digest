...
# Потолок сообщений за один заход отправителя: 5 (до 25 новостей) → 2 (до 10)
# → 1. Причина последнего шага (2026-09-23): даже два сообщения подряд читаются
# как простыня. Вечером 21.09 выпуск ушёл семью новостями в двух сообщениях —
# ручной прогон фильтра днём положил в pending две штуки поверх пяти от
# штатного окна, и потолок в 10 не сработал. Стало 1, то есть ровно MAX_ITEMS
# новостей за выпуск. Всё сверх потолка остаётся pending и придёт следующим
# выпуском: очередь не теряется, а размазывается по двум выпускам в сутки.
# Порог сознательно жёсткий: лучше недосказать, чем завалить.
MAX_MESSAGES = 1
