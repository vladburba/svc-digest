...
def render_filter_line(filter_funnel, waiting):
    ...
    if not is_morning() and len(runs) > 1:
        ...
        took = filter_funnel.get("selected", 0)
        line += (f"\n<i>📊 Итог за сутки: рассмотрено {seen}, "
                 f"отобрано {took} · {total} "
...
def render_funnel(funnel, filter_funnel=None, waiting=0, total=0, overflow=0):
    """Футер сообщения. Правило одно: КАЖДАЯ СТРОКА ЗАМКНУТА САМА НА СЕБЯ.
      ...
      📬 сколько новостей в этой пачке и сколько ждёт следующей;
      🤖 по каждому заходу отбора: из скольких статей сколько выбрано;
      📊 (вечером) итог суток — рассмотрено, отобрано, потрачено запросов.
